...
Technologies used: pnpm, Typescript, an externally provisioned Convex deployment, and Docker deployment for the web and storage services. This repository consumes Convex but does not create or manage the Convex instance. You will also need to document how to architecture the docker file system mounts for user-based storage. No tailwind. CSS should be modularized with the goal to be overridden by galleries/uploader settings.
...
* login should be handled via simple google SSO. Use this implementation as an example: https://github.com/whenwhenwhenwhenwhen/when/blob/main/src/lib/googleAuth.tsx . This is an existing example of anonymous cookie-based session and google SSO based accounts and the merging of such.
